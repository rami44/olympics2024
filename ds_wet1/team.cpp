...
void Team::update_strength(int old_strength, int new_strength, BinaryTree<int, int> &tree) {
    // find node with old strength
    Node<int, int> *node = tree.get_node_from_key(old_strength);
    if (*node->get_data() == 1) {
        tree.delete_key(old_strength);
    } else {
        (*node->get_data())--;
    }
    // find node with new strength
    Node<int, int> *node2 = tree.get_node_from_key(new_strength);
    if (node2 != nullptr) {
        (*node2->get_data())++;
    } else { // if new strength is not in tree, insert it with data 1
        std::shared_ptr<int> one = std::make_shared<int>(1);
        tree.insert_key_and_data(new_strength, one);
    }
}
...
void Team::remove_contestant(int contestantId) {
    ...
    if (node1 != nullptr) { // if contestant is in first tree
        update_strength(node1->get_data()->get_strength(), -1, first_third_strength);
    ...
    if (node2 != nullptr) { // if contestant is in second tree
        update_strength(node2->get_data()->get_strength(), -1, second_third_strength);
        //after updating strength, we may need to rearrange between trees
    ...
    if (node3 != nullptr) { // if contestant is in third tree
        update_strength(node3->get_data()->get_strength(), -1, third_third_strength);
        //after updating strength, we may need to rearrange between trees
...
void Team::move_node(BinaryTree<int, Contestant> &sourceTree_id, BinaryTree<int, Contestant> &destTree_id,
    ...
    // insert strength to new tree
    if (desTree_strength.get_node_from_key(strength) == nullptr) {
        std::shared_ptr<int> one = std::make_shared<int>(1);
        desTree_strength.insert_key_and_data(strength, one);
    } else {
        Node<int, int> *node = desTree_strength.get_node_from_key(strength);
        (*node->get_data())++;
    }
}
