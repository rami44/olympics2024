...
public:
    // Constructor
    BinaryTree() : root(nullptr), numOfNodes(0), maxNode(nullptr), minNode(nullptr) {}
    ...
    // Getter methods
    const Node<Key, Data> *get_root() const {
        return root;
    }

    Node<Key, Data> *get_root() {
        return root;
    }
...
//    }
    ...
    Node<Key, Data> *get_minNode() const {
        return minNode;
    }
    ...
    void update_minNode(Node<Key, Data> *curr) {
        if (curr == nullptr) return;
        minNode = curr;
        update_minNode(curr->get_left());
    }

    // Other methods...

    bool node_exists(const Node<Key, Data> *node,
                     const Node<Key, Data> *curr) {
        if (curr == nullptr)
            return false;
        Key k = node->get_key();
        if (k == curr->get_key())
    ...
    // In-order traversal
    template<class Func>
    void in_order(Func functionPointer, const Node<Key, Data> *curr) {
        if (curr == nullptr) return;
    ...
    // Pre-order traversal
    template<class Func>
    void pre_order(Func functionPointer, const Node<Key, Data> *curr) {
        if (curr == nullptr) return;
        functionPointer(curr);  // Do something with the node
...
///////////////////////////////////////////////
    ...
    // Rolling methods...
    // LR rotation
    void LR_rolling(Node<Key, Data> *rolling_node) {
    ...
    // LL rotation
    void LL_rolling(Node<Key, Data> *rolling_node) {
        Node<Key, Data> *root_father = rolling_node->get_father();
        Node<Key, Data> *new_root = rolling_node->get_left();
    ...
    // RL rotation
    void RL_rolling(Node<Key, Data> *rolling_node) {
        Node<Key, Data> *root_father = rolling_node->get_father();
    ...
    // RR rotation
    void RR_rolling(Node<Key, Data> *rolling_node) {
        Node<Key, Data> *root_father = rolling_node->get_father();
        Node<Key, Data> *new_root = rolling_node->get_right();
        if (root_father != nullptr) { // rolling_node is not the root
            if ((root_father->get_left() != nullptr) && (*(root_father->get_left()) == *rolling_node)) {
                root_father->set_left(new_root);
            } else {
    ...
    bool insert_key_and_data(const Key &key, std::shared_ptr<Data> &data) {
        if (get_node_from_key(key) == nullptr) {
            Node<Key, Data> *new_node =
                    new Node<Key, Data>(key, data);
    ...
    Node<Key, Data> *get_node_from_key(const Key &key) {
        Node<Key, Data> *tmp = root;
        while (tmp != nullptr) {
            if (tmp->get_key() == key) {
                return tmp;
    ...
    bool delete_key(const Key &key) {
        Node<Key, Data> *node = get_node_from_key(key);
